...
do
 ...
 mpirun -np $i --mca pml ucx ./ring_non_blocking.x
 mpirun -np $i --mca pml ucx ./ring_blocking.x
 ...
 mpirun -np $i -mca pml ob1 --mca btl tcp ./ring_non_blocking.x 
 mpirun -np $i -mca pml ob1 --mca btl tcp ./ring_blocking.x

 str=$(cat output_ring_non_blocking | grep time | cut -f2 -d ':')
 str1=$(cat output_ring_blocking | grep time | cut -f2 -d ':')
 time_nb_ob1=$(echo ${str} | cut -d ' ' -f1)
 time_b_ob1=$(echo ${str1} | cut -f2 -d ' ')
 printf '%d,\t%s,\t%s,\t%s,\t%s\n' ${i} ${time_nb[0]} ${time_b[0]}  ${time_nb_ob1} ${time_b_ob1}>> results.csv
