...
void matrix_topo(int dims[3], int mat_sizes[3]){
  ...
  if(rank == MASTER){
    FILE *out;
    out=fopen("output_matrix_topo", "w");
    fprintf(out,"grid: (%d %d %d),matrix sizes: %dx%dx%d, mintime: %10.8f \n",dims[0],dims[1],dims[2], mat_sizes[0], mat_sizes[1],mat_sizes[2], min_time);
    fclose(out);
  }
}
...
void matrix_no_topo(int mat_size[3]){
  ...
  if(rank == MASTER){
   FILE *out;
    out=fopen("output_matrix_no_topo", "w");
    fprintf(out,"dim matrix: %dx%dx%d,mintime: %10.8f \n", mat_size[0], mat_size[1],mat_size[2], min_time);
    fclose(out);
  }
}
